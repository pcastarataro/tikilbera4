...
ParserEntradaEstandar::ParserEntradaEstandar(int newContent_length, char* ruta) {
	...
	nombreArchivoPrincipal = "";  // su length es CERO
...
void ParserEntradaEstandar::analizarLinea(char* cadena) {
	...
	if (boundaryCoincide(&lineaStr))
		...
		else esFinDeUnoDeLosArchivos = true;
	else {
...
int ParserEntradaEstandar::crearArchivosAPartirDeEntradaEstandar() {
	...
	int retorno = 0;
	if (!finDePost) retorno = -1;
	else retorno = listaDeNombresDeArchivos.size();
	return retorno;
}
