...
HTTP_Response* ProcesadorCGI::procesarCGI(HTTP_Request* request,
	...
	if (arch.fail()) {
		...
		cod_retorno = chdir("../");
		string cadenaArchivo = CargardorDeArchivos::levantarArchivo(
				config.getError(501));
		...
		prepararComandoDeTerminal(rutaCGI, dirAbsolutaDelRecurso,
				escribirEnLaTerminal, rutaCompletaArchEntradaTemp,
				rutaCompletaArchSalidaTemp, dirArchivosDeSalida);
		crearArchivoTemporalParaSTDIN(rutaCompletaArchEntradaTemp,
		...
		removerArchivosYDirectoriosTemporales(rutaCompletaArchEntradaTemp,
				rutaCompletaArchSalidaTemp, dirArchivosDeSalida);
	}
...
void ProcesadorCGI::removerArchivosYDirectoriosTemporales(
		const string rutaArchEntrada, const string rutaArchSalida,
...
void ProcesadorCGI::crearArchivoTemporalParaSTDIN(
...
void ProcesadorCGI::prepararComandoDeTerminal(const string rutaCGI,
		const string& uri, string& comando, const string& nombreEntradaTemp,
...
HTTP_Response* ProcesadorCGI::procesar(HTTP_Request* pedido) {
	std::string
			dirAbsolutaDelRecurso(config.getConfiguracionBasica().getRaiz());
