...
int main(int argc, char** argv) {
	...
	if (argc < 3)
		cout << "PARSER_PHP: cantidad de parametros erronea: " << argc
				<< ". Deberia ser igual a 3" << endl;
	else {
		...
		string rutaPHP;
		chdir("/");
		bool parseoParametrosOK = procesador.extraerParametrosParaElPHP(
				rutaPHP, parametros, listaParams);
		if (parseoParametrosOK)
			system(procesador.armarComandoPHP(rutaPHP, listaParams).c_str());
		else
			cout << "error, el recurso no es .php: " << argv[2] << endl;
	}
	return 0;
}
