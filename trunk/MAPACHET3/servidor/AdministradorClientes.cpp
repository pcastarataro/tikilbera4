...
AdministradorClientes::AdministradorClientes(const Configuracion& c): config(c) {
	total = 0;
}
...
void AdministradorClientes::avisarSobrecargaServidor(ManejadorClienteHTTP *manejador) {
...
Lock l(mutex);
    ...
    if(archError != ""){
        try {
            std::string cadena = CargardorDeArchivos::levantarArchivo(archError);
...
void AdministradorClientes::agregarCliente(ManejadorClienteHTTP* manejador) {
	...
	if(total < config.getConfiguracionBasica().getMaximoConexiones()) {
		...
		if(it != this->mapaIpConexiones.end()) {
			...
			if( tamLista < config.getConfiguracionBasica().getMaximoConexionesCliente()) {
...
void AdministradorClientes::limpiarFinalizados() {
    Lock l(mutex);
    ITERADOR itMapa;
    for(itMapa = mapaIpConexiones.begin() ; itMapa != mapaIpConexiones.end() ; itMapa++) {
...
void AdministradorClientes::limpiarActivos() {
	Lock l(mutex);
	ITERADOR itMapa;
	for(itMapa = this->mapaIpConexiones.begin() ; itMapa != this->mapaIpConexiones.end() ; itMapa++) {
...
void AdministradorClientes::limpiarActivosConTimeOutMayor() {
	...
	for(itMapa = this->mapaIpConexiones.begin() ; itMapa != this->mapaIpConexiones.end() ; itMapa++) {
		...
		for(it = itMapa->second.begin() ; it != itMapa->second.end() ; it++) {
			...
			int sec = petm->tm_hour*3600 + petm->tm_min * 60 + petm->tm_sec; // offset Segundos
			int dif = sec - (*it)->getOffsetSegundos();
			if( dif >= config.getConfiguracionBasica().getTimeOut()) {
				ManejadorLogs::getInstance(config.getConfiguracionLogs())->
						getLogError()->logError("Se saca al cliente " + (*it)->getIpCliente()
								+ " por timeout.");
				(*it)->morir();
				(*it)->apagar();
				(*it)->join();
