...
ProcesadorTipoEstatico::ProcesadorTipoEstatico(const Configuracion& c): config(c) {
}
...
HTTP_Response* ProcesadorTipoEstatico::procesar(HTTP_Request* pedido) {
	...
	if(tipo == "") {
		try {
			...
			std::string cadenaArchivo = CargardorDeArchivos::levantarArchivo(config.getError(CODIGO_PAGINA_NO_ENCONTRADA));
			respuesta->setContenido(cadenaArchivo);
		} catch (const ErrorArchivoException&) {
			ManejadorLogs::getInstance(config.getConfiguracionLogs())->
					getLogError()->logError("El archivo " + config.getError(CODIGO_PAGINA_NO_ENCONTRADA) +
	...
	try {
		...
		try {
			ManejadorLogs::getInstance(config.getConfiguracionLogs())->
					getLogError()->logError("El archivo " + pathArchivo +
							" no existe en el servidor");
			cadenaArchivo = CargardorDeArchivos::levantarArchivo(config.getError(404));
			respuesta->setContenido(cadenaArchivo);
			respuesta->setCodigoRetorno(CODIGO_PAGINA_NO_ENCONTRADA);
