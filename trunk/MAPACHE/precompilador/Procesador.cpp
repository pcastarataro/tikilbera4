...
void Procesador::procesarEndif(char* linea) {
...
void Procesador::procesarDefine(char* linea) {
	...
	if (!util.esNumero(ptrA1erParametro)) {
		char vectorDeTopes[3] = { BLANCO, BARRA_N, BARRA_R };
		int largoDelDefine = util.getLargoCadena(ptrA1erParametro,
				vectorDeTopes, 3);
		...
		if (largoDelDefine > LONGITUD_VALOR_DEFINE || largoValorDefine
				> LONGITUD_VALOR_DEFINE)
			flags.statusDelPrograma = ERROR_PROGRAMA;
		else
...
bool Procesador::esSeparadorValidoParaLaInstruccion(char* instr, char sep) {
	...
	if (strcmp(instr, DEFINE) == 0 || strcmp(instr, INCLUDE) == 0 || strcmp(
			instr, IFDEF) == 0 || strcmp(instr, IFNDEF) == 0)
		sepValido1 = BLANCO;

	else if (strcmp(instr, ELSE) == 0 || strcmp(instr, ENDIF) == 0) {
		sepValido1 = BLANCO;
		sepValido2 = BARRA_N;
		sepExtra = true;
	}

	if (!sepExtra && sepValido1 == sep)
		retorno = true;
	else if (sepExtra && (sepValido1 == sep || sepValido2 == sep))
		retorno = true;
	else
		retorno = false;
...
void Procesador::procesarInstruccion(char* linea, int codInstr) {
	...
	else if (codInstr == COD_INSTR_INCLUDE)
		procesarInclude(linea);
	else if (codInstr == COD_INSTR_IFDEF)
		procesarIfdefOIfndef(linea, COD_INSTR_IFDEF);
	else if (codInstr == COD_INSTR_IFNDEF)
		procesarIfdefOIfndef(linea, COD_INSTR_IFNDEF);
	else if (codInstr == COD_INSTR_ELSE)
		procesarElse(linea);
	else if (codInstr == COD_INSTR_ENDIF)
		procesarEndif(linea);
}
...
char* Procesador::procesarDirectivaCompilador(char* linea) {
	...
	char* vecDeInstr[CANT_DE_INTR_DE_COMPILADOR] = { (char*) DEFINE,
			(char*) ELSE, (char*) ENDIF, (char*) IFDEF, (char*) IFNDEF,
			(char*) INCLUDE };

	int vecCod[CANT_DE_INTR_DE_COMPILADOR] = { COD_INSTR_DEFINE,
			COD_INSTR_ELSE, COD_INSTR_ENDIF, COD_INSTR_IFDEF, COD_INSTR_IFNDEF,
			COD_INSTR_INCLUDE };

	while (!esLaInstrCorrecta && contador < CANT_DE_INTR_DE_COMPILADOR) {
		esLaInstrCorrecta = parseInstruccion(linea, vecDeInstr[contador],
				vecCod[contador]);
		contador++;
	}
...
void Procesador::procesoArchivoEntrada(const char* nombreArchivo) {
	...
	if (ptrFile != NULL) {
		...
		cout << "Se trato de incluir al archivo " << str
				<< ", pero no existe en el servidor" << endl;
		cout << "Se lo busco en " << dir << endl;
		cout << "Causas del error: no se envio el archivo ";
		cout << "requerido o el servidor fallo al crearlo";
		cout << endl;
		free(dir);
	}
